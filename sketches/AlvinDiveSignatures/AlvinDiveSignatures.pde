...
Alvin Dive Data Visualizer
blprnt@blprnt.com
May, 2014

Required libraries: controlP5
Also, you'll need to replace the fonts with ones that you have on hyour machine.

Notes:
  - Video mode will take up a lot of space since it's recording to TIF files. You can switch the extension to .jpg or .png but it'll be much slower.
  - 4720 currently doesn't load (I think because of a missing depth file?)
...
String alvinServer = "/Volumes/OCR LockBox/AlvinDump/ALVIN/";
...
void loadInterface() {
  ...
  cp5.addToggle("playing")
     .setPosition(30,height - 85)
     .setSize(50,20)
     ;
     
  // DRAW LINE
  cp5.addToggle("drawLine")
     .setPosition(90,height - 85)
     .setSize(50,20)
     ;
  
  // DEPTH
  cp5.addToggle("hasDepth")
     .setPosition(150,height - 85)
     .setSize(50,20)
     ;
     
  // VIDEO RECORD
  cp5.addToggle("recVideo")
     .setPosition(650,height - 85)
     .setSize(50,20)
     ;
...
void draw() {
  ...
  if (!controlled){
   ...
   textSize(48);
   fill(255);
   if (frameCount < 90) text("Trying to connect to Alvin data.", 30, 280); 
   else if (frameCount > 110) {
     text("Couldn't connect to Alvin data.", 30, 280);
     text("Are you connected to 192.168.11.242/data_on_alvin ?", 30, 325);
     text("If you are, press the spacebar to try again.", 30, 380);
  ...
  if (video) {
    saveFrame("Video/" + cruiseID + featureDive.diveNo + "/" + videoCount + ".tif"); 
    videoCount ++;
  }
}
...
void keyPressed() {
  ...
  if (key == ' ') {
    ...
    if (!controlled) thread("loadInterface");
  }
  if (key == 'd') {
    tdeep = (tdeep == 0) ? 1:0;
    if (tdeep == 1 && trot.x == 0) {
